...
def analyze_layers_batched(
    ...
    # Process each shape group with batched SVD
    for shape, group_layers in shape_groups.items():
        batch_size = len(group_layers)
        print(f"Processing shape {shape}: {batch_size} layers...")
        
        # Stack weights into batch
        weight_batch = torch.stack([m.weight.data for _, m in group_layers])
        ...
        for i, (layer_name, _) in enumerate(group_layers):
            print(f"  [{i+1}/{batch_size}] {layer_name}")
